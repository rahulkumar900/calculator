import numberToWord from "@/app/number-to-word/page";
...
const nw = (function () {
  ...
  const GF = function GWF(arg,f) {
    ...
    const lw = f === "currency" ? toWord(ln) + " dollar" : toWord(ln);
    ...
    const rw = 
    rn && Number(rn) > 0
    ? `and ${toWord(rn)} ${f === "currency" ? "cents" : points[rn.length].toLowerCase()}`
    : "";
  ...
  const IF = function (arg,f) {
    ...
    const lw = f === "currency" ?  toWord(ln) + " rupees" : toWord(ln);
    const prefix = lw ? "and" : "";
    const rw =
      rn && Number(rn) > 0
        ? `${prefix} ${toWord(rn)} ${f === 'currency' ? '' : Ipoints[rn.length].toLowerCase()} ${ f === 'currency' ? "paise" : '' }`
        : "";
